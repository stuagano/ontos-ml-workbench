#!/bin/bash
# Bootstrap script for Ontos ML Workbench
# Usage: ./scripts/bootstrap.sh <workspace-name> [--profile <profile>]
#
# This script:
...
if [ -z "$WORKSPACE_NAME" ]; then
    echo "Usage: $0 <workspace-name> [--profile <profile>]"
    echo ""
    echo "Example: $0 my-workspace --profile my-profile"
    echo ""
    echo "If --profile is not specified, workspace-name is used as the profile name."
    exit 1
fi
...
if ! databricks auth profiles 2>/dev/null | grep -q "$PROFILE_NAME.*YES"; then
    ...
    log_warn "Please authenticate first:"
    log_warn "  databricks auth login https://your-workspace.cloud.databricks.com --profile=$PROFILE_NAME"
    exit 1
fi
...
if ! databricks auth profiles 2>/dev/null | grep -q "$PROFILE_NAME.*YES"; then
    ...
    log_error "  databricks auth login https://your-workspace.cloud.databricks.com --profile=$PROFILE_NAME"
    exit 1
fi
