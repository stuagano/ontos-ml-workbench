...
github: https://github.com/databricks-field-eng/mirion-workspace/issues/7
depends_on: [2, 6]
parallel: false
conflicts_with: []
---
...
3. Clicks Training Sheet → sees Q&A pairs
4. For each pair:
   - View: Question + Answer + Source Data (image/PDF/table)
   - Actions: Approve ✓ | Edit ✏️ | Reject ✗ | Flag 🚩
...
- [ ] Keyboard shortcuts: A (approve), E (edit), R (reject), F (flag), → (next), ← (prev)
...
├── TrainingSheetList.tsx        # List of Training Sheets
├── QAPairReviewer.tsx           # Individual Q&A pair review
├── MultimodalDisplay.tsx        # Image/PDF/table rendering
├── QAPairActions.tsx            # Action buttons (Approve/Edit/Reject/Flag)
├── InlineEditor.tsx             # Edit assistant response
├── BulkActionsBar.tsx           # Batch operations
└── ProgressIndicator.tsx        # Review progress display
```
...
```
frontend/src/
├── pages/LabelStage.tsx
├── components/label/
│   ├── TrainingSheetList.tsx
│   ├── QAPairReviewer.tsx
