...
def _lakebase_table(name: str) -> str:
    """Get fully qualified Lakebase table name using configured catalog and schema."""
    s = get_settings()
    return f"`{s.databricks_catalog}`.`{s.lakebase_schema}`.`{name}`"
...
async def get_curated_dataset(dataset_id: str):
    ...
    result = _sql.execute(
        f"""
        SELECT * FROM {_lakebase_table('curated_datasets')} WHERE id = ?
    """,
        [dataset_id],
    )
...
async def update_curated_dataset(
    ...
    _sql.execute(
        f"""
        UPDATE {_lakebase_table('curated_datasets')}
...
async def delete_curated_dataset(dataset_id: str):
    ...
    _sql.execute(
        f"""
        DELETE FROM {_lakebase_table('curated_datasets')} WHERE id = ?
    """,
        [dataset_id],
    )
...
async def list_curated_datasets(
    ...
    count_result = _sql.execute(
        f"""
        SELECT COUNT(*) as total FROM {_lakebase_table('curated_datasets')} {where_clause}
    """,
        params,
    )
    ...
    results = _sql.execute(
        f"""
        SELECT * FROM {_lakebase_table('curated_datasets')}
        {where_clause}
