...
CREATE INDEX IF NOT EXISTS idx_qa_pairs_training_sheet ON ${CATALOG}.${SCHEMA}.qa_pairs(training_sheet_id);
CREATE INDEX IF NOT EXISTS idx_qa_pairs_review_status ON ${CATALOG}.${SCHEMA}.qa_pairs(review_status);
CREATE INDEX IF NOT EXISTS idx_qa_pairs_canonical_label ON ${CATALOG}.${SCHEMA}.qa_pairs(canonical_label_id) WHERE canonical_label_id IS NOT NULL;
CREATE INDEX IF NOT EXISTS idx_qa_pairs_auto_approved ON ${CATALOG}.${SCHEMA}.qa_pairs(was_auto_approved) WHERE was_auto_approved = true;
CREATE INDEX IF NOT EXISTS idx_qa_pairs_item_ref ON ${CATALOG}.${SCHEMA}.qa_pairs(sheet_id, item_ref);
