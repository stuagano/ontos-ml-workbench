...
VALUES
  ...
  -- Lineage / graph traversal tools
  ('mcp-tool-009', 'traverse_lineage', 'Traverse upstream or downstream lineage from an entity', 'governance', '{"type":"object","properties":{"entity_type":{"type":"string","enum":["sheet","template","training_sheet","model","endpoint"]},"entity_id":{"type":"string"},"direction":{"type":"string","enum":["upstream","downstream"]},"max_depth":{"type":"integer","default":10}},"required":["entity_type","entity_id","direction"]}', 'read', 'governance:read', true, '1.0', '/api/v1/governance/graph/traverse/{direction}/{entity_type}/{entity_id}', CURRENT_TIMESTAMP(), 'system', CURRENT_TIMESTAMP(), 'system'),
  ('mcp-tool-010', 'impact_analysis', 'Analyze blast radius of changes to an entity', 'governance', '{"type":"object","properties":{"entity_type":{"type":"string","enum":["sheet","template","training_sheet","model","endpoint"]},"entity_id":{"type":"string"}},"required":["entity_type","entity_id"]}', 'read', 'governance:read', true, '1.0', '/api/v1/governance/graph/impact/{entity_type}/{entity_id}', CURRENT_TIMESTAMP(), 'system', CURRENT_TIMESTAMP(), 'system'),
  ('mcp-tool-011', 'find_related_assets', 'Find immediate neighbors of an entity in the lineage graph', 'governance', '{"type":"object","properties":{"entity_type":{"type":"string","enum":["sheet","template","training_sheet","model","endpoint"]},"entity_id":{"type":"string"}},"required":["entity_type","entity_id"]}', 'read', 'governance:read', true, '1.0', '/api/v1/governance/graph/context/{entity_type}/{entity_id}', CURRENT_TIMESTAMP(), 'system', CURRENT_TIMESTAMP(), 'system');
