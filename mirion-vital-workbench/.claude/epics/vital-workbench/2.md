...
**Schema Location:** `home_stuart_gano.mirion_vital_workbench` (using home catalog instead of creating new catalog due to permissions)
...
Create all Delta Lake table schemas in Unity Catalog for the VITAL Platform Workbench. This includes the core data model for Sheets, Templates, Canonical Labels, Training Sheets, Q&A Pairs, and Model Training Lineage. These tables form the foundation for the entire platform.

**Location:** `mirion_vital.workbench` catalog

## Acceptance Criteria

- [ ] `mirion_vital.workbench` catalog exists in Unity Catalog
...
- [ ] `mirion_vital` catalog exists (or create it)
...
- [ ] Tables created in Unity Catalog `mirion_vital.workbench`
- [ ] Verified via `SHOW TABLES IN mirion_vital.workbench`
