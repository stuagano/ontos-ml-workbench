...
## Key UI Patterns
- **Stage Navigation**: Linear workflow (DATA → GENERATE → LABEL → TRAIN → DEPLOY → MONITOR → IMPROVE). Note: DATA+GENERATE are intentionally merged into a single "Datasets" sidebar entry (SheetBuilder handles both).
